...
const navItems = [
  { label: 'Copilot dashboard', icon: 'dashboard' },
        ...
        <ul className="space-y-1 mb-7">
          {navItems.map((item) => {
            ...
            return (
              <li key={item.label}>
                <button
                  className={`w-full flex items-center gap-3 text-left px-4 py-2 rounded-lg group relative
                    ${isActive
                  ...
                  onClick={() => onSectionChange && onSectionChange(item.label)}
                ...
                >
                  <span className="w-6 h-6 flex items-center justify-center">
                    ...
                    {item.icon === 'reports' && (
                      <svg width="22" height="22" fill="none" viewBox="0 0 22 22" className={isActive ? 'text-indigo-600' : 'text-gray-400'}><rect x="4" y="4" width="14" height="14" rx="3" fill="currentColor"/><rect x="7" y="7" width="8" height="2" rx="1" fill="#fff"/><rect x="7" y="11" width="5" height="2" rx="1" fill="#fff"/></svg>
                    )}
                  </span>
                  {item.label}
                </button>
